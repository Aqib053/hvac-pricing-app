...
def create_margin(data: MarginCreate, db: Session = Depends(get_db)):
    repo = MarginRepository(db)
    return repo.create(
...
def update_margin(margin_id: int, data: MarginUpdate, db: Session = Depends(get_db)):
    repo = MarginRepository(db)
...
def delete_margin(margin_id: int, db: Session = Depends(get_db)):
    repo = MarginRepository(db)
    margin = repo.get_by_id(margin_id)
    if not margin:
        raise HTTPException(status_code=404, detail="Margin not found")
